...
| OQ-4 | Name/branding — is "vdodtor" final? | **Closed — settled 2026-09-03.** The name ships. The icon is the app's own timeline, three clip bars under the playhead, generated by `tools/make_icon.dart` rather than vendored, so it carries no letterform and nothing would have to be redrawn if §1 ever moved. The site is in `site/` in the repository — not because marketing belongs beside an engine, but because §3's "no updater" means the six `vdodtor.app` addresses compiled into a build can never be changed afterwards, so the two have to be tested together. What remains is a domain and a host, which are not decisions |
| OQ-5 | Distribution: where do the first 1,000 users come from? | Open. The "no watermark video editor for Mac" page it asks for exists — it is `site/index.html` — so what is left is the channels rather than the page |
