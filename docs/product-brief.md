...
| OQ-4 | Name/branding — is "vdodtor" final? | **Yes — settled 2026-09-03.** The name ships. The icon is settled with it: the app's own timeline, three clip bars under the playhead, generated by `tools/make_icon.dart` rather than vendored, so it carries no letterform and nothing about it would have to be redrawn if §1 ever moved. The **website with a direct download is still outstanding**, and is the whole of what is left of OQ-4 |
